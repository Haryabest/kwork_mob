...
def test_pick_three_seed_views_keeps_middle(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Регрессия: при 3 фото не пропускать среднюю (view_04)."""
    ...
    _write(photos / "view_06.png", b"side-unique-bytes-bbb")
...
def test_pick_does_not_linspace_skip_middle(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    _write(photos / "view_04.png", b"b")
    _write(photos / "view_08.png", b"c")
    ...
    picked = pick_input_images(photos, task_dir=tmp_path)
    names = [p.name for p in picked]
    assert names == ["view_00.png", "view_04.png"]
    assert "view_08.png" not in names
