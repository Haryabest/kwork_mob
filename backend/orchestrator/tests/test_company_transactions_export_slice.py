...
from app.services.company_balance import transaction_status, transactions_to_csv
...
def test_transactions_to_csv_header_and_row():
    ...
    assert lines[0] == "id,user_id,date,type,amount,status,description"
    assert "42" in lines[1]
    assert "charge" in lines[1]
    assert "-500" in lines[1]
    assert "Успешно" in lines[1]


def test_transaction_status_failed_and_pending():
    failed = SimpleNamespace(description="Ошибка возврата ЮKassa")
    pending = SimpleNamespace(description="Ожидание подтверждения")
    ok = SimpleNamespace(description="Пополнение баланса")
    assert transaction_status(failed) == "failed"
    assert transaction_status(pending) == "pending"
    assert transaction_status(ok) == "succeeded"
