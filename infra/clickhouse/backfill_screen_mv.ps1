# Backfill mobile_analytics_screen_daily on existing ClickHouse §19.20
param(
...
$sqlPath = Join-Path $PSScriptRoot "backfill_mobile_analytics_screen_daily.sql"
$sql = Get-Content $sqlPath -Raw -Encoding UTF8
$uri = "http://${ChHost}:${ChPort}/?database=$ChDb"

Write-Host "backfill mobile_analytics_screen_daily -> $uri"
$headers = @{}
if ($ChPassword) {
